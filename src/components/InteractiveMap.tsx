...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  useEffect(() => {
    ...
    farms.forEach((farm) => {
      ...
      markerElement.className = `w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 relative ${
        isHighlighted 
          ? 'bg-green-600 shadow-lg shadow-green-400/50 scale-110' 
      ...
      tooltip.className = 'absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-card border border-border rounded-lg shadow-lg opacity-0 pointer-events-none transition-opacity duration-200 whitespace-nowrap z-50';
      tooltip.innerHTML = `
        <div class="text-sm font-medium text-foreground">${farm.name}</div>
